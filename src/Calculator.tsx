...
export default class Calculator extends React.Component<CalculatorProps, CalculatorState> {
    ...
    calculateStateProp(state: CalculatorState, prop: CalculatorValue, fn: Function): CalculatorState {
    ...
    handleChange(prop: CalculatorValue, value: number) {
        this.setState(prevState => {
            ...
            nextState = this.calculateStateProp(nextState, CalculatorValue.DurationInSeconds, () => {
                return this.calculateDuration(nextState[CalculatorValue.DistanceInMeters], nextState[CalculatorValue.PaceInSeconds])
            })

            nextState = this.calculateStateProp(nextState, CalculatorValue.DistanceInMeters, () => {
                return this.calculateDistance(nextState[CalculatorValue.DurationInSeconds], nextState[CalculatorValue.PaceInSeconds])
            })

            nextState = this.calculateStateProp(nextState, CalculatorValue.PaceInSeconds, () => {
                return this.calculatePace(nextState[CalculatorValue.DurationInSeconds], nextState[CalculatorValue.DistanceInMeters])
            })
